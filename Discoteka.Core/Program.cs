...
class Program
{
    static void Main(string[] args)
    {
        if (args.Length < 1)
        {
            ...
            Console.WriteLine("  Discoteka.Core xml <path-to-xml>");
            Console.WriteLine("  Discoteka.Core scan <path-to-music-folder>");
            Console.WriteLine("  Discoteka.Core clean [--confidence <0-100>] [--dry-run]");
            Console.WriteLine("  Discoteka.Core match [--dry-run]");
            return;
        }
        ...
        switch (command)
        {
            case "xml":
                if (args.Length < 2)
                {
                    Console.WriteLine("Usage: Discoteka.Core xml <path-to-xml>");
                    return;
                }
                RunXmlImport(args[1]);
                break;
            case "scan":
                if (args.Length < 2)
                {
                    Console.WriteLine("Usage: Discoteka.Core scan <path-to-music-folder>");
                    return;
                }
                RunFileScan(args[1]);
                break;
