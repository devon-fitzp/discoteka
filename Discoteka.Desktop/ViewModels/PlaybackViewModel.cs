...
public sealed class PlaybackViewModel : ViewModelBase, IDisposable
{
    ...
    // Accessed from VLC's internal thread — use Interlocked/volatile for visibility.
    private long _currentTrackIdValue; // 0 = no current track; access via Interlocked
    private volatile bool _currentTrackCounted;
    ...
    private void OnCurrentTrackChanged(PlaybackTrack? track)
    {
        ...
        Interlocked.Exchange(ref _currentTrackIdValue, track?.TrackId ?? 0L);
    ...
    {
        var trackId = Interlocked.Read(ref _currentTrackIdValue);
        if (!state.IsPlaying || _currentTrackCounted || trackId <= 0)
        {
            return;
        }
